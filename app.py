...
# --- 安全配置：改用 st.secrets ---
# 部署後請在 Streamlit Cloud 控制台設定此金鑰
try:
...
def get_mstr_fundamentals():
    ...
    try:
        ...
        if not shares or not debt:
            raise ValueError("API 數據不完整")
            
        return float(shares), float(debt), float(preferred), float(cash or 2250000000.0)
    except Exception as e:
        st.warning(f"基本面數據抓取異常，使用基準值。")
...
try:
    ...
    if not mstr_close.empty and not btc_close.empty:
        ...
        with col_den:
            ...
            st.caption(f"持倉量: {mstr_btc_holdings:,.0f} BTC")

        # 3. 圖表
        render_charts(df, selected_metrics)
    else:
        st.error("無法載入數據，請檢查 API 配額。")
